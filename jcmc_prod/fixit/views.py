...
def execute(JobId, scriptnames, hosts_file):
    global monitor_fie
    CURRENT_TIME = time.strftime('%d%m%Y_%H%m%S')
    LogFile = f'/tmp/JC_FIX_Log_{CURRENT_TIME}'
    monitor_fie = f'{LogFile}_monitor'

    # cmd = f'/pdsfsnadmin/scripts/jcmate/scan_hosts_adoc_6.py  --file {hosts_file} --manual --enable --parse_passwd {USER_PASSWD} --logfile {LogFile}  --cmd "{scriptnames.strip()}" --timeout {int( TIMEOUT) * 60} --regex_item "{DROPDM_REGEX}" --regex_value "{REGEX_VALUE}" --last_lines {LASTLOGTOSHOW} &'


    cmd = f'/scratch/srgadi/scan_hosts_adoc_7.py  --file {hosts_file} --manual --enable --parse_passwd {USER_PASSWD} --logfile {LogFile}  --cmd "{scriptnames.strip()}" --timeout {int( TIMEOUT) * 60} --regex_item "{DROPDM_REGEX}" --regex_value "{REGEX_VALUE}" --last_lines {LASTLOGTOSHOW} &'
    print('Executing the command\n', cmd)
    os.system(cmd)
    time.sleep(5)

    '''
    It will update the status of issuesed command to Database as and when the logfile have the new data, copying the original log file to two duplicate copies and erasing the data from original
    one for reference and other for updating the DB which have only latest information from log.
    '''
    while True:
        if os.path.exists(monitor_fie):
            print('=> 1.Processing\n')
            with open(LogFile, 'r') as org, open(f'{LogFile}.tmp', 'w') as tmp:
                t_org = org.readlines()
                tmp.writelines(i for i in t_org)

            # Erasing the data from orginal log file, to have latest data only.
            with open(LogFile, 'w') as org:
                pass

            with open(f'{LogFile}.tmp', 'r') as record:
                print(f'=>READING {LogFile}.tmp')
                # pdb.set_trace()
                for entry in record.readlines():
                    if entry:
                        print('=>Entry ', entry)
                        try:
                                hostname = entry.split('|')[1].strip()
                                # Result = '|'.join(entry.split('|')[2:]).strip()

                                # Fetching the last host which was save with home_view module to update the status

                                latest_record_id = HostList.objects.filter(Hostname__contains=hostname).values().last()['id']
                                update_record = HostList.objects.get(id=latest_record_id)

                            # if update_record.Result:
                            #     update_record.Result = update_record.Result + '\n' + Result
                            # else:
                                # updating the latets host executed results to DB
                                State = entry.split('|')[0]
                                Result = entry.split('|')[2:][0]
                                # Result = Result

                                if Result.count('OK') == 4:
                                    update_record.State = 'Fixed'
                                    update_record.Result = Result

                                elif 'mismatch ' in Result:
                                    update_record.Result = 'No space left on root File System'
                                    update_record.State = State

                                elif 'EOF during negotiation' in Result:
                                    update_record.Result = '/pdsfsnadmin/scripts/jcmc/jc_package_installation.py ||/usr/bin/curl -s http://pd-wls-adc-01.us.oracle.com/scripts/jcmc/jc_package_installation.py |python'
                                    update_record.State = 'Manual'
                                else:
                                    update_record.State = State
                                    update_record.Result = Result

                                update_record.save()
                                print(f'\n{hostname} : DB updated successfully.')
...
def Analysis(JobId, hostnames, username, scriptnames, hosts_file, SPLIT_SIZE):
    ...
    for turn, division_hosts in enumerate(list(chunk(set(hostnames), SPLIT_SIZE))):
        ...
        # print(f'==>Sleeping for {int(turn)*60} min to start a turn batch')
        # time.sleep(int(turn)*60)
        time.sleep(10)
...
def API_db_update(request):
    ...
    '''Finding for dup'''
    # try:
    # patch_report.objects.update_or_create( host_name__icontains=patchData['host_name',
    #
    #      patch_report.objects.create(Pid=JobId,
    #                                  State = State,
...
def view_status(request, pid=None, job_state=None):
    ...
    if pid and job_state:
        if job_state == 'NotSuccess':
            query_results = HostList.objects.filter(Pid=pid).exclude(Q(State__icontains='Success')).exclude(Q(State__icontains='Running')).exclude(Q(State__icontains='InvalidHost')).order_by('-id')
...
def view_advSearch(request):
    ...
    if request.POST:
        ...
        found_search_results = patch_report.objects.filter(Q(host_name__in=SearchkeyWord)).order_by('-DateTime').values()
